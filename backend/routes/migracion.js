...
router.get('/preview', async (req, res) => {
  try {
    ...
    const preview = await migracionService.previewMigracion(anioEscolar);
...
router.post('/promover', async (req, res) => {
  try {
    ...
    const resultados = await migracionService.promoverAlumnos(alumnosIds, anioEscolar);
...
router.post('/graduar', async (req, res) => {
  try {
    ...
    await migracionService.graduarAlumno(alumnoId, anioEscolar || new Date().getFullYear());
...
router.post('/retirar', async (req, res) => {
  try {
    ...
    await migracionService.retirarAlumno(
      alumnoId,
      motivo,
...
router.post('/fin-de-anio', async (req, res) => {
  try {
    ...
    logger.info({ anioEscolar }, 'Iniciando migración masiva de fin de año');
    
    const resultados = await migracionService.migracionFinDeAnio(anioEscolar);
...
router.get('/historial/:id', async (req, res) => {
  try {
    ...
    const historial = await migracionService.getHistorialAlumno(alumnoId);
